...
const list: any[] = []
listData?.value?.char29Data.forEach((data: any)=> list.push(data.resourceSetName))
if(list.indexOf(route.params.id) === -1) {
  throwError()
}
...
const {pending: isFetchLoading, data: imgData} = await useFetch(`/api/getImgArray?resourceSetName=${route.params.id}`, {lazy: true, server: false})
